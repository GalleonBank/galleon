...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GLN unit input in lnReqAmount, we generate the URI with GLN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::GLN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
